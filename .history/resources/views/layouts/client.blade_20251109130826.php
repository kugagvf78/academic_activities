...
<body class="font-inter bg-gray-50 text-gray-800 flex flex-col min-h-screen">
    ...
    {{-- HEADER / NAVBAR --}}
    <header class="bg-white/90 backdrop-blur-xl border-b border-gray-100 sticky top-0 z-50 shadow-sm">
        <div class="container mx-auto flex items-center justify-between px-6 py-4 lg:py-5">
            ...
            <a href="{{ route('client.home') }}" class="flex items-center space-x-3 group">
                <div class="w-12 h-12 rounded-2xl bg-gradient-to-tr from-blue-600 via-cyan-500 to-blue-400 flex items-center justify-center shadow-md group-hover:scale-110 transition-transform duration-300">
                    <i class="fa-solid fa-graduation-cap text-white text-xl"></i>
                </div>
                <div class="leading-tight">
                    <h1 class="text-xl font-extrabold text-gray-800 tracking-tight group-hover:text-blue-700 transition-colors">
                        Hệ thống Hội thảo CNTT
                    </h1>
            ...
            <nav class="hidden md:flex items-center space-x-8 font-medium text-gray-700">
                <a href="{{ route('client.home') }}"
                    class="relative group flex items-center gap-2 hover:text-blue-600 transition">
                    <i class="fa-solid fa-home text-blue-500"></i>
                    <span>Trang chủ</span>
                    <span class="absolute -bottom-1 left-0 w-0 h-[2px] bg-gradient-to-r from-blue-500 to-cyan-400 rounded-full group-hover:w-full transition-all"></span>
                </a>
            ...
            <div class="flex items-center space-x-3">
                @auth
                <div class="hidden md:flex items-center bg-gray-50 rounded-xl px-3 py-2 border border-gray-200 shadow-sm">
                    <div class="w-8 h-8 bg-gradient-to-tr from-blue-600 to-cyan-500 rounded-full flex items-center justify-center mr-2 shadow-inner">
                        <i class="fa-solid fa-user text-white text-xs"></i>
                    </div>
                    <span class="text-sm font-medium text-gray-700">{{ Auth::user()->name ?? 'Người dùng' }}</span>
                </div>

                <form action="{{ route('logout') }}" method="POST" class="inline">
                    @csrf
                    <button class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-semibold text-sm hover:shadow-lg hover:scale-105 transition-all flex items-center gap-2">
                ...
                @else
                <a href="{{ route('login') }}"
                    class="px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-semibold text-sm hover:shadow-lg hover:scale-105 transition-all flex items-center gap-2">
    ...
    <main class="flex-1">
        {{-- HERO SECTION --}}
        <header class="bg-white/95 backdrop-blur-2xl border-b border-gray-100 sticky top-0 z-50 shadow-sm transition-all duration-300">
            <div class="container mx-auto flex items-center justify-between px-5 py-4 lg:px-8 lg:py-5">
                {{-- LOGO + BRAND --}}
                <a href="{{ route('client.home') }}" class="group flex items-center space-x-3.5 transition-transform duration-300 hover:scale-[1.02]">
                    <div class="relative w-12 h-12 rounded-2xl bg-gradient-to-tr from-blue-600 via-cyan-500 to-blue-400 p-[2px] shadow-lg group-hover:shadow-xl transition-all duration-300">
                        <div class="w-full h-full rounded-2xl bg-white/90 flex items-center justify-center">
                            <i class="fa-solid fa-graduation-cap text-blue-600 text-xl"></i>
                        </div>
                    </div>
                    <div class="leading-tight">
                        <h1 class="text-xl font-black text-gray-900 tracking-tighter group-hover:text-blue-700 transition-colors duration-300">
                            Hội thảo CNTT
                        </h1>
                        <p class="text-xs font-semibold text-cyan-600 tracking-wide">Khoa Công nghệ Thông tin</p>
                    </div>
                </a>

                {{-- NAVIGATION - Desktop --}}
                <nav class="hidden md:flex items-center space-x-10 font-semibold text-gray-700">
                    @php
                    $navItems = [
                    ...
                    <a href="{{ route($item['route']) }}"
                        class="group relative flex items-center gap-2.5 py-2 px-1 text-sm hover:text-blue-600 transition-all duration-300 {{ request()->routeIs($item['route']) ? 'text-blue-600' : '' }}">
                        @else
                        <a href="{{ $item['href'] }}"
                            class="group relative flex items-center gap-2.5 py-2 px-1 text-sm hover:text-blue-600 transition-all duration-300">
                            @endif
                            <i class="fa-solid {{ $item['icon'] }} text-blue-500 text-base transition-transform group-hover:scale-110"></i>
                            <span class="font-medium">{{ $item['label'] }}</span>
                            <span class="absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-blue-500 to-cyan-400 rounded-full group-hover:w-full transition-all duration-300"></span>
                        </a>
                        @endforeach
                </nav>
                ...
                <div class="hidden md:flex items-center space-x-4">
                    @auth
                    <div class="flex items-center bg-gradient-to-r from-blue-50 to-cyan-50 rounded-2xl px-4 py-2.5 border border-blue-200/50 shadow-sm hover:shadow-md transition-all duration-300">
                        <div class="w-9 h-9 rounded-full bg-gradient-to-tr from-blue-600 to-cyan-500 flex items-center justify-center shadow-inner">
                            <i class="fa-solid fa-user text-white text-sm"></i>
                        </div>
                        <span class="ml-2.5 text-sm font-semibold text-gray-800">{{ Str::limit(Auth::user()->name, 15) }}</span>
                    </div>
                    <form action="{{ route('logout') }}" method="POST" class="inline">
                        @csrf
                        <button class="group flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-red-500 to-pink-500 text-white font-semibold text-sm shadow-md hover:shadow-xl hover:scale-105 transition-all duration-300">
                    ...
                    @else
                    <a href="{{ route('login') }}"
                        class="group flex items-center gap-2.5 px-6 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-bold text-sm shadow-lg hover:shadow-xl hover:scale-105 transition-all duration-300">
            ...
            {{-- MOBILE MENU --}}
            <div class="md:hidden hidden bg-white/95 backdrop-blur-xl border-t border-gray-100 shadow-xl" id="mobile-menu">
                <div class="container mx-auto px-6 py-5 space-y-4">
                    @foreach ($navItems as $item)
                    @if (isset($item['route']))
                    <a href="{{ route($item['route']) }}"
                        class="flex items-center gap-3 py-3 px-4 rounded-xl hover:bg-blue-50 transition-colors {{ request()->routeIs($item['route']) ? 'bg-blue-50 text-blue-600' : 'text-gray-700' }}">
                        @else
                        <a href="{{ $item['href'] }}"
                            class="flex items-center gap-3 py-3 px-4 rounded-xl hover:bg-blue-50 transition-colors text-gray-700">
                            @endif
                        ...
                        <div class="pt-4 border-t border-gray-200">
                            @auth
                            <div class="flex items-center justify-between py-3 px-4 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-600 to-cyan-500 flex items-center justify-center">
                                        <i class="fa-solid fa-user text-white text-sm"></i>
                                    </div>
                                    <span class="font-bold text-gray-800">{{ Str::limit(Auth::user()->name, 18) }}</span>
                                </div>
                                <form action="{{ route('logout') }}" method="POST" class="inline">
                                    @csrf
                                    <button class="text-red-500 font-semibold text-sm hover:text-red-600 transition-colors">
                                        Đăng xuất
                                    </button>
                                </form>
                            </div>
                            @else
                            <a href="{{ route('login') }}"
                                class="block text-center py-3 px-6 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-bold shadow-lg hover:shadow-xl transition-all">
                                Đăng nhập
                            </a>
                            @endauth
                        </div>
        ...
        {{-- Thêm Animate.css (tùy chọn) để hiệu ứng mượt hơn --}}
