...
============================================================ */
    public function register(Request $request)
    {
        try {
            ...
            $request->validate([
                'macuocthi' => 'required',
                'type' => 'required|in:individual,team',   // mobile gửi "type"
            ]);
            ...
            /* ============================================================
        |  CASE 1: ĐĂNG KÝ CÁ NHÂN
        ============================================================ */
            if ($request->type === "individual") {

                $request->validate([
                    'leader_student_code' => 'required',
                ]);

                // Kiểm tra trùng đăng ký
                $exists = DB::table('dangkycanhan')
                    ->where('macuocthi', $request->macuocthi)
                    ->where('masinhvien', $request->leader_student_code)
                    ->exists();
                ...
                DB::table('dangkycanhan')->insert([
                    ...
                    'masinhvien' => $request->leader_student_code,
                    'ngaydangky' => now(),
                    'trangthai' => 'Registered'
                ]);

                DB::commit();
                return response()->json(['success' => true, 'message' => "Đăng ký cá nhân thành công"]);
            }

            /* ============================================================
        |  CASE 2: ĐĂNG KÝ ĐỘI NHÓM
        ============================================================ */
            ...
            // ===== Kiểm tra trưởng nhóm có đăng ký trước đó không =====
            $leaderRegistered = DB::table('dangkydoithi')
                ->join('doithi', 'dangkydoithi.madoithi', '=', 'doithi.madoithi')
                ->where('dangkydoithi.macuocthi', $request->macuocthi)
                ->where('doithi.matruongdoi', $request->leader_student_code)
                ->exists();

            if ($leaderRegistered) {
                return response()->json(['success' => false, 'message' => 'Bạn đã đăng ký đội cho cuộc thi này'], 400);
            }

            // ===== 1. Tạo mã đội thi =====
            $teamId = 'DT' . strtoupper(uniqid());

            DB::table('doithi')->insert([
                'madoithi' => $teamId,
                'tendoithi' => $request->team_name,
                'matruongdoi' => $request->leader_student_code,
                'ngaytao' => now(),
            ]);

            // ===== 2. Lưu TRƯỞNG ĐỘI vào bảng thành viên =====
            DB::table('thanhviendoithi')->insert([
                'madoithi' => $teamId,
                'masinhvien' => $request->leader_student_code,
                'hoten' => $request->leader_name,
                'email' => $request->leader_email,
                'isLeader' => true,
            ]);

            // ===== 3. Lưu CÁC THÀNH VIÊN KHÁC =====
            if ($request->has('members') && is_array($request->members)) {
                foreach ($request->members as $m) {
                    if (!empty($m['student_code'])) {
                        DB::table('thanhviendoithi')->insert([
                            'madoithi' => $teamId,
                            'masinhvien' => $m['student_code'],
                            'hoten' => $m['name'] ?? '',
                            'email' => $m['email'] ?? '',
                            'isLeader' => false,
                        ]);
