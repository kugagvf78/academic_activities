...
<body class="font-oswald bg-gray-50 text-gray-800 flex flex-col min-h-screen">
    ...
    {{-- HEADER --}}
<header x-data="{ openMenu: false }" class="bg-white backdrop-blur-md shadow sticky top-0 z-50 border-b border-gray-100">
    <div class="container mx-auto flex justify-between items-center py-4 px-6">
        ...
        <button @click="openMenu = !openMenu"
            class="md:hidden text-gray-700 hover:text-blue-600 focus:outline-none transition">
            <i :class="openMenu ? 'fa-solid fa-xmark text-2xl' : 'fa-solid fa-bars text-2xl'"></i>
        </button>
        ...
        <nav class="hidden md:flex space-x-8 font-medium">
            ...
            <a href="{{ route('client.events') }}"
                class="nav-link hover:text-blue-600 {{ request()->routeIs('client.events') ? 'active text-blue-600 font-semibold' : '' }}">
                Cuộc thi
            </a>
            <a href="#"
                class="nav-link hover:text-blue-600 {{ request()->is('results*') ? 'active text-blue-600 font-semibold' : '' }}">
                Kết quả
            </a>
            <a href="#"
        ...
        <div class="hidden md:flex items-center space-x-4">
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false"
                    class="absolute right-0 mt-3 w-52 bg-white border border-gray-300 rounded-xl overflow-hidden shadow">
    ...
    {{-- 🔽 Menu di động --}}
    <div x-show="openMenu" x-transition class="md:hidden bg-white border-t border-gray-200 shadow-inner">
        <nav class="flex flex-col space-y-2 py-4 px-6 text-gray-700 font-medium">
            ...
            <a href="{{ route('password.change') }}" class="hover:text-blue-600">Hồ sơ cá nhân</a>
            <form action="{{ route('logout') }}" method="POST">
                @csrf
                <button type="submit" class="text-left text-red-600 hover:underline mt-2">Đăng xuất</button>
            </form>
            @else
            <a href="{{ route('login') }}" class="text-blue-600 font-semibold hover:underline mt-2">Đăng nhập</a>
            @endauth
        </nav>
    </div>
</header>
