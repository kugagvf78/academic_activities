...
<body class="font-oswald bg-gray-50 text-gray-800 flex flex-col min-h-screen">
    ...
    <header x-data="{ openMenu: false }" class="bg-white shadow sticky top-0 z-50 border-b border-gray-100">
                ...
                <div class="relative" x-data="{ openUser: false }">
                    ...
                    <div x-show="openUser" @click.away="openUser = false"
                        ...
                        <a href="{{ route('password.change') }}"
                            class="block px-4 py-3 text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition">Hồ sơ cá
                            nhân</a>
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button type="submit"
                                class="w-full text-left px-4 py-3 text-red-600 hover:bg-red-50 font-semibold transition">Đăng
                                xuất</button>
                        </form>
                    </div>
                </div>
        ...
        {{-- 📱 Menu dọc (hiện khi nhấn hamburger) --}}
        <div x-show="openMenu" x-transition.duration.300ms
            ...
            <nav class="flex flex-col px-6 py-4 space-y-3 font-medium text-gray-700">
                ...
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="text-left text-red-600 hover:underline mt-1">Đăng xuất</button>
                </form>
                @else
                <hr class="border-gray-200 my-2">
                <a href="{{ route('login') }}" class="text-blue-600 font-semibold hover:underline">Đăng nhập</a>
                @endauth
            </nav>
        </div>
    </header>
