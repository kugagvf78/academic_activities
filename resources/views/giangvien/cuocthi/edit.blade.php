...
<section class="container mx-auto px-6 py-6">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h1 class="text-3xl font-bold text-gray-800">Chỉnh sửa cuộc thi</h1>
        </div>
        <p class="text-gray-600 ml-10">Cập nhật thông tin cuộc thi: <strong>{{ $cuocthi->tencuocthi }}</strong></p>
    </div>

    {{-- Form --}}
    <div class="bg-white rounded-lg shadow-md p-8">
        <form method="POST" action="{{ route('giangvien.cuocthi.update', $cuocthi->macuocthi) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <option value="CuocThi" 
                    {{ old('loaicuocthi', $cuocthi->loaicuocthi) == 'CuocThi' ? 'selected' : '' }}>
                    Cuộc thi
                </option>
                
                <option value="HoiThao" 
                    {{ old('loaicuocthi', $cuocthi->loaicuocthi) == 'HoiThao' ? 'selected' : '' }}>
                    Hội thảo
                </option>
