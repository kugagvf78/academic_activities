...
<section x-data="{ active: 0, slides: [
    { img: '{{ asset('images/home/banner1.jpg') }}', title: 'Khám Phá Tri Thức – Tỏa Sáng Tài Năng', subtitle: 'Cuộc thi học thuật dành cho sinh viên Khoa CNTT', button: 'Tham Gia Ngay' },
    { img: '{{ asset('images/home/banner2.png') }}', title: 'Nơi Kết Nối Sinh Viên Đam Mê Công Nghệ', subtitle: 'Học hỏi – Cạnh tranh – Phát triển cùng bạn bè đồng khoa', button: 'Xem Cuộc Thi' },
    { img: '{{ asset('images/home/banner3.jpg') }}', title: 'Vinh Danh Thành Tích – Ghi Dấu Thành Công', subtitle: 'Những gương mặt xuất sắc trong hành trình học thuật CNTT', button: 'Xem Thành Tích' }
    ...
    <template x-for="(slide, index) in slides" :key="index">
        <div x-show="active === index"
            ...
            <div class="absolute inset-0 flex flex-col items-center justify-center text-center text-white px-6">
                <h4 class="uppercase tracking-wider font-semibold text-blue-200 text-sm md:text-base mb-3">
                    Khoa Công Nghệ Thông Tin
                </h4>
                <h1 x-text="slide.title"
                    class="text-4xl md:text-6xl font-extrabold leading-tight mb-4 drop-shadow-lg"></h1>
                <p x-text="slide.subtitle"
                    class="text-base md:text-lg text-blue-100 mb-6 max-w-2xl"></p>
                <a href="{{ route('client.events') }}"
                    x-text="slide.button"
                    class="inline-block bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-700 hover:to-cyan-600 px-8 py-3.5 rounded-xl font-semibold shadow-lg transition-all hover:scale-105">
                </a>
            </div>
        </div>
    </template>
    ...
    <button @click="active = active === 0 ? slides.length - 1 : active - 1"
        class="absolute left-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/30 text-white rounded-full w-10 h-10 flex items-center justify-center">
        <i class="fa-solid fa-chevron-left"></i>
    </button>

    <button @click="active = active === slides.length - 1 ? 0 : active + 1"
        class="absolute right-4 top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/30 text-white rounded-full w-10 h-10 flex items-center justify-center">
