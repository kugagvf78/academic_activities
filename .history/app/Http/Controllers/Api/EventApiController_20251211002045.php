    ...
    public function show($macuocthi)
    {
        try {
            ...
            if (!$user) {
                return response()->json([
                    'success' => false,
                    'message' => 'User chưa đăng nhập hoặc token không hợp lệ',
                ], 401);
            }

            $masinhvien = $user->profile->masinhvien ?? $user->tendangnhap ?? null;

            if (!$masinhvien) {
                return response()->json([
                    'success' => false,
                    'message' => 'Không tìm thấy mã sinh viên của user',
                ], 400);
            }
            ...
            // Đăng ký dự thi (cá nhân hoặc đội)
            $daDangKyDuThi =
                ...
                ->where('macuocthi', $macuocthi)
                ->whereIn('madoithi', function ($q) use ($masinhvien) {
                    $q->select('madoithi')
                        ->from('thanhviendoi')
                        ->where('masinhvien', $masinhvien);
                })
            ...
            // Đăng ký hỗ trợ
            $daDangKyHotro = DB::table('dangkyhoatdong as dk')
                ->where('dk.masinhvien', $masinhvien)
                ->whereIn('dk.mahoatdong', function ($q) use ($macuocthi) {
                    $q->select('mahoatdong')->from('hoatdonghotro')->where('macuocthi', $macuocthi);
                })
                ->exists();

            // Đăng ký cổ vũ
            $daDangKyCoVu = DB::table('dangkycolvu as dk')
                ->where('dk.masinhvien', $masinhvien)
                ->whereIn('dk.mahoatdong', function ($q) use ($macuocthi) {
                    $q->select('mahoatdong')->from('hoatdonghotro')->where('macuocthi', $macuocthi);
                })
            ...
            $eventData = [
                ...
                // 🔥 THÊM 3 TRƯỜNG QUAN TRỌNG ĐÚNG YÊU CẦU
                'dadangky_duthi' => $daDangKyDuThi,
                'dadangky_hotro' => $daDangKyHotro,
                'dadangky_colvu' => $daDangKyCoVu,
            ];
