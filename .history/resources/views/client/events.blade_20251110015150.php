...
<section class="container mx-auto px-6 py-16">
    ...
    {{-- 📄 PAGINATION --}}
    @if($events->hasPages())
    <div class="mt-16 mx-5 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center space-x-1">
            ...
            @else
            <a href="{{ $events->url(1) }}"
                class="flex items-center justify-center w-9 h-9 rounded-lg bg-white border border-gray-200 text-blue-600 hover:bg-blue-50 transition">
                <i class="fas fa-angle-double-left"></i>
            </a>
            @endif
            ...
            <span class="flex items-center justify-center w-9 h-9 rounded-lg bg-gradient-to-r from-blue-600 to-cyan-500 text-white font-semibold shadow-sm">
                {{ $page }}
            </span>
            @else
            <a href="{{ $url }}"
        ...
        {{-- 🔹 Bên phải: mô tả số lượng / per page --}}
        <div class="flex items-center gap-3 text-sm text-gray-600">
            ...
            <form method="GET" id="perPageForm" class="flex items-center gap-2">
                ...
                <span>cuộc thi / trang</span>
            </form>

            {{-- Hiển thị “1–6 trong tổng 42” --}}
            @php
            $from = ($events->currentPage() - 1) * $events->perPage() + 1;
            $to = min($events->currentPage() * $events->perPage(), $events->total());
            @endphp
            <span class="text-gray-500">
                Hiển thị {{ $from }}–{{ $to }} / {{ $events->total() }}
            </span>
        </div>
    </div>
    @else
    ...
    <div class="bg-gradient-to-br from-gray-50 to-blue-50 rounded-2xl shadow-sm border border-gray-200 p-16 text-center">
        <div class="max-w-md mx-auto">
            <i class="fas fa-calendar-xmark text-8xl text-gray-300 mb-4"></i>
            <h4 class="text-2xl font-bold text-gray-700 mb-3">Không tìm thấy cuộc thi</h4>
            <p class="text-gray-500 mb-6 leading-relaxed">Không có cuộc thi nào phù hợp với tiêu chí tìm kiếm của bạn.</p>
            <a href="{{ route('client.events') }}"
                class="inline-flex items-center bg-gradient-to-r from-blue-600 to-cyan-500 text-white px-6 py-3 rounded-xl font-semibold hover:shadow-lg transition">
